    ...
    <el-row>
      <el-col :span="24">
        <el-breadcrumb separator="/" style="margin-bottom:10px">
          <el-breadcrumb-item class="nav-back" @click="onGoHome">
            首页456
          </el-breadcrumb-item>
          <el-breadcrumb-item>
            ...
            <el-tooltip placement="top" v-for="item of nodeList" :key="item.nodeId" :visible-arrow="false">
              <template #content>
                <div>
                  ...
                  <p>最新采集时间:{{ item.acquisitionTime || '暂无数据' }}</p>
                </div>
              </template>
              <!-- <span
    ...
    let METHODS: any = reactive({
                    ...
                    key.warningSectionBusWayDtos.forEach((val: any) => {
                      ...
                      if (
                        ...
                        val.homePageNodeDtoList.forEach((v: any) => {
                          // v.x = (DATA.imgScaling * v.x) / 100; //根据图片缩放 生成新的比例
      ...
      handleNodedrag: (e: any, item: any) => {
        ...
        document.onmousemove = (e: any) => {
          if (isDown == false || DATA.isLock === false) {
            return;
          }
          ...
          try {
            if (data.data != null) {
              ...
              if (
                ...
                // data.data.nodeDates = [{...data.data.nodeDates[0]},{collectionTemp:25.1,collectionDate: "2022/12/28 9:58:43"}]
      ...
      handleReset: () => {
        ...
        DATA.nodeSize = DATA.nodeAspect.size;
      ...
      handleMouseWheel: () => {
        newNodeList.value = JSON.parse(JSON.stringify(DATA.nodeList));
        buswayImg.value.addEventListener(
          "wheel",
          function (e: any) {
            ...
            if (evt.deltaY > 0) {
              DATA.imgScaling -= 1;
              DATA.nodeList.forEach((item: any) => {
                newNodeList.value.forEach((val: any) => {
                  if (item.nodeId === val.nodeId) {
                    item.x = (val.x * DATA.imgScaling) / 100;
                  }
                });
              });
              buswayImg.value.style.width = DATA.imgScaling + "%";
            } else {
              DATA.imgScaling += 1;
              DATA.nodeList.forEach((item: any) => {
                newNodeList.value.forEach((val: any) => {
